...
arteRouter.get("/:id", async (req: any, res: any) => {
	try {
		...
		if (arte.show === false)
			return res
				.status(404)
				.json({ message: "arte no disponible para mostrar" });
...
arteRouter.get("/private/:id", userAuth, async (req: any, res: any) => {
	try {
		const arte = await Arte.findById(req.params.id);
		...
		if (arte?.uid === req.uid)
			return res
				.status(401)
				.json({ message: "Este arte no le pertenece" });
...
arteRouter.put("/:id", userAuth, async (req: any, res: any) => {
	try {
		...
		const updatedarte = await Arte.findByIdAndUpdate(
			req.params.id,
			req.body,
			{ new: true }
		);
